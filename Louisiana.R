...
data <- read.csv("NEW_ORLEANS_AIRPORT_LA.csv")
...
mtext("Profile Log-Likelihood Plots for PRCP (LA)", outer = TRUE, cex = 1.2)
...
mtext("Profile Log-Likelihood Plots for WSF5 (LA)", outer = TRUE, cex = 1.2)
...
mtext("Profile Log-Likelihood Plots for Water Depth (LA)", outer = TRUE, cex = 1.2)
...
# loc       scale     shape 
# 3.8614744 1.1220217 0.2634813 

print(gev_fit_WSF5$estimate)
# loc      scale     shape 
# 54.17716037  7.68310935  0.02495886

print(gev_fit_waterDepth$estimate)
# loc        scale      shape 
# 2.2145931 0.3488864 0.4513889
...
# For PRCP
create_gpd_stability_plots(final_combined_data, "PRCP", "LA")

# For WSF5
create_gpd_stability_plots(final_combined_data, "WSF5", "LA")

# For average_waterDepth
create_gpd_stability_plots(final_combined_data, "average_waterDepth", "LA")
...
# Selected thresholds
selected_threshold_PRCP <- 1.5
selected_threshold_WSF5 <- 46
selected_threshold_average_waterDepth <- 1.3
...
create_gpd_fit_plots <- function(data, variable_name, threshold, return_period = 100) {
  ...
  grid.arrange(p1, p2, p3, p4, ncol = 2,
               top = textGrob(paste("GPD Fit for", variable_name, "at threshold =", threshold, "(LA)"),
                              gp = gpar(fontsize = 16, font = 2)))
...
cat("95% confidence intervals for Water Depth (LA):\n")
print(conf_intervals_waterDepth)
...
plot_empirical_vs_fitted_contours <- function(u_data, fitted_copula, var_names) {
  ...
  # Add a common title
  mtext("Empirical vs Fitted Copula Contour Plots (LA)", outer = TRUE, cex = 1.5)
}
...
generate_conditional_samples <- function(wind_speed, copula, n_samples = 1000) {
  ...
  if (is.na(u_wind) || u_wind < 0 || u_wind > 1) {
    message("Invalid u_wind value for wind speed ", wind_speed, ": ", u_wind)
    return(data.frame(PRCP = NA, WaterDepth = NA))
  }
  
  # Generate samples from the copula
  samples <- rCopula(n_samples, copula)
  
  # Conditioning: Select samples where the wind component is close to u_wind
  tolerance <- 0.05  # Adjust as needed
  valid_samples <- samples[abs(samples[, 2] - u_wind) < tolerance, ]
  
  if (nrow(valid_samples) < 10) {
    message("Not enough valid conditional samples generated for wind speed ", wind_speed)
    return(data.frame(PRCP = NA, WaterDepth = NA))
  }
  
  # Transform back to original scales
  rain <- qgpd(valid_samples[, 1], 
               loc = selected_threshold_PRCP, 
               scale = prcp_fit$estimate["scale"], 
               shape = prcp_fit$estimate["shape"])
  
  flood <- qgpd(valid_samples[, 3], 
...
rainfall_plot <- ggplot(results_df, aes(x = wind_speed)) +
  ...
  labs(title = "Mean Rainfall vs Wind Speed (LA)",
       x = "Wind Speed",
       y = "Rainfall") +
  theme_minimal()
...
flood_plot <- ggplot(results_df, aes(x = wind_speed)) +
  ...
  labs(title = "Mean Flood Level vs Wind Speed (LA)",
       x = "Wind Speed",
       y = "Flood Level") +
  theme_minimal()
...
prob_exceed_LA <- joint_exceedance(original_scale_data, thresholds)
print(paste("Probability of joint exceedance (LA):", prob_exceed_LA))

# Visualize the simulated data
pairs(original_scale_data, main="Simulated Extreme Events (LA)")
...
# Filter the dataset for rain-related damages and state LA
filtered_rain_only <- Flood_Insurance_Loss %>% 
  filter(causeOfDamage == "4") # & state == "LA"
...
# Filter the dataset for state LA and buildingDamageAmount > 0
filtered_dataset_LA_non_profit <- filtered_dataset %>% 
  filter(state == "LA", nonProfitIndicator == "0", buildingDamageAmount > 1000, buildingDamageAmount < 100000)

# Find the row with the highest buildingDamageAmount and remove it
filtered_dataset_LA_non_profit <- filtered_dataset_LA_non_profit %>%
  filter(buildingDamageAmount != max(buildingDamageAmount, na.rm = TRUE))

# Read the weather data
weather_data <- read.csv("NEW_ORLEANS_AIRPORT_LA.csv")
...
# View the final combined data
head(filtered_dataset_LA_non_profit)
...
for (var in c(factor_vars, numeric_vars)) {
  if (is.factor(filtered_dataset_LA_non_profit[[var]])) {
    if (length(unique(filtered_dataset_LA_non_profit[[var]])) > 1) {
...
result <- forward_select_pvalue(filtered_dataset_LA_non_profit, "buildingDamageAmount", all_predictors)
...
model_LA_non_profit <- glm(buildingDamageAmount ~ 
                             elevatedBuildingIndicator +
                             numberOfFloorsInTheInsuredBuilding +
                             buildingAgeAtLoss +
                             basementEnclosureCrawlspaceType +
                             ...
                             WSF5 +
                             PRCP,
                           data = filtered_dataset_LA_non_profit, 
                           family = Gamma(link = "log"))

summary(model_LA_non_profit)



str(filtered_dataset_LA_non_profit[, c("basementEnclosureCrawlspaceType", 
                                       "elevatedBuildingIndicator", 
                                       "locationOfContents", 
                                       "numberOfFloorsInTheInsuredBuilding", 
                                       "floodWaterDuration", 
...
new_data_glm_non_profit <- data.frame(
  basementEnclosureCrawlspaceType = factor(c(1, 0, 0), levels = levels(filtered_dataset_LA_non_profit$basementEnclosureCrawlspaceType)),
  elevatedBuildingIndicator = factor(c(0, 0, 0), levels = levels(filtered_dataset_LA_non_profit$elevatedBuildingIndicator)),
  #locationOfContents = factor(c(2, 4, 6), levels = levels(filtered_dataset_LA_non_profit$locationOfContents)),
  numberOfFloorsInTheInsuredBuilding = as.integer(c(2, 3, 2)),  # Keep as integer
...
str(new_data_glm_non_profit)


# Make predictions
predicted_loss_values_glm_non_profit_LA <- predict(model_LA_non_profit, 
                                                   newdata = new_data_glm_non_profit, 
                                                   type = "response")

# Print predictions
print(predicted_loss_values_glm_non_profit_LA)
# 18406.67 30817.53 34945.35 
...
# Prepare the data
model_data <- filtered_dataset_LA_non_profit[, c("buildingDamageAmount", 
                                                 "basementEnclosureCrawlspaceType",
                                                 "elevatedBuildingIndicator",
                                                 #"locationOfContents",
                                                 "numberOfFloorsInTheInsuredBuilding",
...
mean(predicted_loss_values_glm_non_profit_LA)*prob_exceed_LA