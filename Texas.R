...
data <- read.csv("ANGLETON_LAKE_JACKSON_BRAZORIA_CO_AIRPORT.csv")
...
create_profile_plot(gev_fit_PRCP, "loc", "Profile Log-Likelihood of Loc (PRCP)")
create_profile_plot(gev_fit_PRCP, "scale", "Profile Log-Likelihood of Scale (PRCP)")
create_profile_plot(gev_fit_PRCP, "shape", "Profile Log-Likelihood of Shape (PRCP)")
mtext("Profile Log-Likelihood Plots for PRCP (TX)", outer = TRUE, cex = 1.2)

# WSF5 Plots
par(mfrow = c(1, 3), mar = c(4, 4, 2, 1), oma = c(0, 0, 2, 0))

create_profile_plot(gev_fit_WSF5, "loc", "Profile Log-Likelihood of Loc (WSF5)")
create_profile_plot(gev_fit_WSF5, "scale", "Profile Log-Likelihood of Scale (WSF5)")
create_profile_plot(gev_fit_WSF5, "shape", "Profile Log-Likelihood of Shape (WSF5)")
mtext("Profile Log-Likelihood Plots for WSF5 (TX)", outer = TRUE, cex = 1.2)

# Water Depth Plots
par(mfrow = c(1, 3), mar = c(4, 4, 2, 1), oma = c(0, 0, 2, 0))

create_profile_plot(gev_fit_waterDepth, "loc", "Profile Log-Likelihood of Loc (Water Depth)")
create_profile_plot(gev_fit_waterDepth, "scale", "Profile Log-Likelihood of Scale (Water Depth)")
create_profile_plot(gev_fit_waterDepth, "shape", "Profile Log-Likelihood of Shape (Water Depth)")
mtext("Profile Log-Likelihood Plots for Water Depth (TX)", outer = TRUE, cex = 1.2)
...
# For PRCP
create_gpd_stability_plots(final_combined_data, "PRCP", "TX")

# For WSF5
create_gpd_stability_plots(final_combined_data, "WSF5", "TX")

# For average_waterDepth
create_gpd_stability_plots(final_combined_data, "average_waterDepth", "TX")
...
create_gpd_fit_plots <- function(data, variable_name, threshold, return_period = 100) {
  ...
  grid.arrange(p1, p2, p3, p4, ncol = 2,
               top = textGrob(paste("GPD Fit for", variable_name, "at threshold =", threshold, "(TX)"),
                              gp = gpar(fontsize = 16, font = 2)))
...
cat("95% confidence intervals for Water Depth (TX):\n")
print(conf_intervals_waterDepth)
...
library(MASS)  # Load necessary libraries
library(copula)
...
plot_empirical_vs_fitted_contours <- function(u_data, fitted_copula, var_names) {
  ...
  # Add a common title
  mtext("Empirical vs Fitted Copula Contour Plots (TX)", outer = TRUE, cex = 1.5)
}
...
# create separate plots for rainfall and flood
library(ggplot2)
...
rainfall_plot <- ggplot(results_df, aes(x = wind_speed)) +
  ...
  labs(title = "Mean Rainfall vs Wind Speed (TX)",
       x = "Wind Speed",
       y = "Rainfall") +
  theme_minimal()
...
flood_plot <- ggplot(results_df, aes(x = wind_speed)) +
  ...
  labs(title = "Mean Flood Level vs Wind Speed (TX)",
       x = "Wind Speed",
       y = "Flood Level") +
  theme_minimal()
...
prob_exceed_TX <- joint_exceedance(original_scale_data, thresholds)
print(paste("Probability of joint exceedance (TX):", prob_exceed_TX))

# Visualize the simulated data
pairs(original_scale_data, main="Simulated Extreme Events (TX)")
...
# Filter the dataset for state TX and buildingDamageAmount > 0
filtered_dataset_TX_non_profit <- filtered_dataset %>% 
  filter(state == "TX", nonProfitIndicator =="0", buildingDamageAmount > 1000, buildingDamageAmount < 100000)

# Find the row with the highest buildingDamageAmount and remove it
filtered_dataset_TX_non_profit <- filtered_dataset_TX_non_profit %>%
  filter(buildingDamageAmount != max(buildingDamageAmount, na.rm = TRUE))

# Read the weather data
weather_data <- read.csv("ANGLETON_LAKE_JACKSON_BRAZORIA_CO_AIRPORT.csv")
...
# View the final combined data
head(filtered_dataset_TX_non_profit)
...
# filtered_dataset_TX_non_profit is dataset
data_TX_losses <- filtered_dataset_TX_non_profit$buildingDamageAmount

# Remove zeros and scale down the data
data_TX_losses <- data_TX_losses[data_TX_losses > 0] / 1e3

# Fit the gamma distribution
ans_TX <- fitdist(data_TX_losses, distr = "gamma", method = "mle")
...
plot(ans_TX)
mtext("Building Loss Amount (TX)", side = 3, line = -1.5, outer = TRUE)
...
# Custom forward selection function based on p-values
...
for (var in c(factor_vars, numeric_vars)) {
  if (is.factor(filtered_dataset_TX_non_profit[[var]])) {
    if (length(unique(filtered_dataset_TX_non_profit[[var]])) > 1) {
...
result <- forward_select_pvalue(filtered_dataset_TX_non_profit, "buildingDamageAmount", all_predictors)
...
model_TX_non_profit <- glm(buildingDamageAmount ~ 
                             basementEnclosureCrawlspaceType +
                             elevatedBuildingIndicator +
                             #locationOfContents +
                             #numberOfFloorsInTheInsuredBuilding +
                             ...
                             PRCP +
                             WSF5, 
                           data = filtered_dataset_TX_non_profit, 
                           family = Gamma(link = "log"))

summary(model_TX_non_profit)
...
new_data_glm_non_profit <- new_data_glm_non_profit %>%
  mutate(
    basementEnclosureCrawlspaceType = factor(basementEnclosureCrawlspaceType, levels = levels(filtered_dataset_TX_non_profit$basementEnclosureCrawlspaceType)),
    elevatedBuildingIndicator = factor(elevatedBuildingIndicator, levels = levels(filtered_dataset_TX_non_profit$elevatedBuildingIndicator)),
    locationOfContents = factor(locationOfContents, levels = levels(filtered_dataset_TX_non_profit$locationOfContents)),
    numberOfFloorsInTheInsuredBuilding = factor(numberOfFloorsInTheInsuredBuilding, levels = levels(filtered_dataset_TX_non_profit$numberOfFloorsInTheInsuredBuilding))
  )

# Make predictions
predicted_loss_values_glm_non_profit_TX <- predict(model_TX_non_profit, 
                                                   newdata = new_data_glm_non_profit, 
                                                   type = "response")

# Print predictions
print(predicted_loss_values_glm_non_profit_TX)
# 41642.53 58322.23 57457.08
...
# Prepare the data
model_data <- filtered_dataset_TX_non_profit[, c("buildingDamageAmount", 
                                                 "basementEnclosureCrawlspaceType",
                                                 "elevatedBuildingIndicator",
                                                 #"locationOfContents",
                                                 "numberOfFloorsInTheInsuredBuilding",
...
cv_results <- lapply(folds, function(test_indices) {
  ...
  # Calculate RMSE
  rmse <- rmse_gamma(log(predictions), test_data$buildingDamageAmount)
...
mean(predicted_loss_values_glm_non_profit_TX)*prob_exceed_TX
